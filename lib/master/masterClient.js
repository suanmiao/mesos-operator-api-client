...
function MesosOperatorApiClient (options) {
    ...
    self.allowedEventTypes = [
        "SUBSCRIBED", "TASK_ADDED", "TASK_UPDATED", "AGENT_ADDED", "AGENT_REMOVED", "FRAMEWORK_ADDED", "FRAMEWORK_UPDATED", "FRAMEWORK_REMOVED", "HEARTBEAT"
    ];
    ...
    if (options.masterPort) {
        self.options.masterPort = parseInt(options.masterPort);
    }
...
MesosOperatorApiClient.prototype.subscribe = function () {
    ...
    self.httpRequest.on('error', function (e) {
        if (self.connectionBackoffTime === 0){
            ...
            self.connectionBackoffTime = Math.min(self.connectionBackoffTime * 2, self.maxConnectionBackoffTime);
        }

        self.emit("error", { message: "There was a problem with the request, will retry in " + (self.connectionBackoffTime / 1000) + " seconds. Error message: " + (e.message ? e.message : JSON.stringify(e)) });
